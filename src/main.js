...
/** Blob → ArrayBuffer。iOS Safari 14未満は Blob.arrayBuffer() 非対応のため FileReader を使う */
...
async function playSavedSong(id, triggerBtn = null) {
  ...
  try {
    ...
    const arrayBuffer = await readBlobAsArrayBuffer(record.blob);
...
async function handleAnalyze() {
  ...
  try {
    ...
    // オンセットが少なすぎる場合は BPM ベースの譜面にフォールバック
    if (currentChart.totalNotes < 10) {
      console.warn('[analyze] too few onsets, using BPM fallback');
      const fbChart = generateBPMChart({
        title, bpm: analysis.bpm, durationMs: analysis.durationMs, difficulty,
      });
      currentChart = normalizeChart(fbChart);
    }
